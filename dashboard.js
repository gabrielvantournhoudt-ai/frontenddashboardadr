(function(){
  ...
  function updateSummaryWidgets(adrs, adrsSnapshots, marketData){
    ...
    // Cache diário (fuso ET) para Fechamento (17:00–18:00) e After Hours (17:00–21:00)
    ...
    if (tickers.length && !afterRealtime) {
      console.warn('Sem dados after-hours disponíveis no backend/Supabase para os tickers monitorados.');
    }
  ...
  // Força Fechamento exatamente às 17:00 ET por ticker (congela o valor do fechamento oficial)
  function computeFechamentoAt1700Totals(adrs, snapshots, tickers){
    ...
    // Determinar data "hoje" em ET
    const et = getNowInET();
    const todayKey = et.dayKey;
  ...
  // ===== Daily cache helpers for Fechamento (17:00 ET sharp) and After Hours (17:00–21:00 ET) =====
  function getNowInET(){
    // Approximate ET by using US/Eastern offset against UTC; for simplicity use New York rules via Intl
    try {
      const now = new Date();
      ...
      // Build a Date components object
      ...
      // Create a Date in ET then convert to actual JS Date by parsing as UTC then shifting back using timezone offset is complex.
      // For comparisons we only need local ET components and a day key.
  ...
  function withinWindowET(kind, et){
    // kind: 'fechamento' (17:00–18:00, inclusive start, exclusive end), 'afterhours' (17:00–21:00)
    const h = et.hour;
    if (kind === 'fechamento') {
      // only from 17:00:00 <= t < 18:00:00 ET
      return h === 17; // minute/second ranges are allowed within the hour
  ...
  function wrapWithDailyCache(kind, realtimeTotals, timestampResolver){
    const et = getNowInET();
    const key = cacheKey(kind, et.dayKey);
    const cachedRaw = localStorage.getItem(key);
    const cached = cachedRaw ? JSON.parse(cachedRaw) : null;

    // When inside window, if we have realtime data, (re)store it; if outside, prefer cached if available
    const inWindow = withinWindowET(kind, et);
    ...
    if (inWindow) {
      ...
      // no realtime data; fall back to cached if exists
      if (cached) {
    ...
    // Outside window → serve cached for the day if present; otherwise serve realtime (but do not store)
    if (cached) {
      return { totals: cached.totals, timestamp: cached.timestamp ? new Date(cached.timestamp) : null, cached: true };
    }
    if (realtimeTotals) {
  ...
  async function updateAll(forceUpdate = false){
    setStatus('updating','Atualizando...');
    try {
      // Só chama /api/update se for forçado (botão manual) ou primeira carga
      if (forceUpdate) {
  ...
  function bind(){
    ...
    if (btn) btn.addEventListener('click', () => updateAll(true));
  ...
  // bootstrap
  bind();
  updateAll(true); // Primeira carga: forçar atualização
  
  // Atualização automática a cada 5 minutos (300000ms) - SEM forçar backend
  setInterval(() => updateAll(false), 5 * 60 * 1000);
  console.log('🔄 Atualização automática ativada: a cada 5 minutos (apenas busca dados)');
})();
