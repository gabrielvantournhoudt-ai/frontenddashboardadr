(function(){
  ...
  // Permite configurar a API via variável global injetada no Vercel (por exemplo, <script>window.API_BASE=...</script>)
  const envApi = (window && window.API_BASE) ? String(window.API_BASE) : null;
  const BASE_URL = (customApi || envApi)
    ? (customApi || envApi).replace(/\/?$/, '')
